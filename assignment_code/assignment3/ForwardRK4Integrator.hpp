...
class ForwardRK4Integrator : public IntegratorBase<TSystem, TState> {
  TState Integrate(const TSystem& system,
    ...
    auto k_2 = system.ComputeTimeDerivative(state + dt/2 * k_1, start_time + dt/2);
    auto k_3 = system.ComputeTimeDerivative(state + dt/2 * k_2, start_time + dt/2);
    auto k_4 = system.ComputeTimeDerivative(state + dt * k_3, start_time + dt);
